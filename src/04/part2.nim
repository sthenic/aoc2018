...
proc get_guard_most_likely_minute(
      sleep_table: SleepTable,
      guard_id: int): tuple[minute: range[0..59], frequency: int] =
   ...
   var max: tuple[idx: range[0..59], val: int]
   for idx, val in pairs(count):
      if max.val < val:
         max.val = val
         max.idx = idx

   result = (max.idx, max.val)
...
when is_main_module:
   ...
   var max: tuple[id, frequency: int, minute: range[0..59]]
   for id in sleep_table.guards:
      let (m, f) = get_guard_most_likely_minute(sleep_table, id)
      if max.frequency < f:
         max.frequency = f
         max.id = id
         max.minute = m

   echo "Product is ", max.id * max.minute, "\n"
